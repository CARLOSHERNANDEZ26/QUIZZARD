...
export default function LeaderboardScreen() {
  ...
  const fetchLeaderboard = async () => {
    const { data, error } = await supabase
      .from('match_results')
      .select('username, score, tier, category')
      .order('score', { ascending: false })
      .order('played_at', { ascending: false }) // If scores are tied, newest is higher
      .limit(10); // BACK TO THE TOP 10!
  ...
  useEffect(() => {
    const initTimer = setTimeout(() => fetchLeaderboard(), 0);
  ...
  return (
    <main className="min-h-screen bg-purple-950 flex flex-col items-center justify-start py-12 px-6 relative overflow-hidden">
          ...
          {leaders.length === 0 ? (
            ...
            leaders.map((player, index) => (
              ...
              >
                ...
                <div className={`text-center ${index === 0 ? "text-yellow-400 text-xl md:text-2xl drop-shadow-[0_0_8px_#facc15]" : index === 1 ? "text-slate-300 text-lg md:text-xl" : index === 2 ? "text-orange-500 text-lg md:text-xl" : "text-white/50"}`}>
                  #{index + 1}
                </div>
